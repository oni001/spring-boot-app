...
pipeline { //Start of declerative pipeline 
	...
	parameters { // Parameters to be used in the pipeline
		...
		choice(name: 'EXECUTION_ROLE_ARN', choices: 'arn:aws:iam::681504525746:role/ecsTaskExecutionRole')
	...
	stages { //block of operations to be performed under pipeline
		...
		stage ("Push docker image and clean docker images") { //Push docker image to Nexus repository
			steps {
                ...
                script {
                       sh 'aws ecr get-login-password --region us-east-1 | docker login --username AWS --password-stdin 681504525746.dkr.ecr.us-east-1.amazonaws.com'
			         //  customImage.push()
		...
		stage('Deploy Image to ECS') {
            steps{
                ...
                script {
                    ...
                    sh ("aws ecs register-task-definition --region us-east-1  --output json --cli-input-json file://${WORKSPACE}/taskdef_${SHORT_COMMIT}.json > ${env.WORKSPACE}/temp.json")
                    def projects = readJSON file: "${env.WORKSPACE}/temp.json"
                    def TASK_REVISION = projects.taskDefinition.revision

                    // update service
                    sh ("aws ecs update-service --cluster ${CLUSTER_NAME} --service ${SERVICE_NAME} --task-definition ${TASK_FAMILY}:${TASK_REVISION} --desired-count 1 --region us-east-1")
                }
            }
        }
	}
